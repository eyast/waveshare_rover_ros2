...
void i2c_init(uint8_t sda_pin, uint8_t scl_pin, uint32_t frequency) {
    // WHAT: Initialize the Wire library (Arduino's I2C interface)
    // WHY: Must be called before using I2C, sets up hardware
    Wire.begin(sda_pin, scl_pin);
    
    // WHAT: Set communication speed
    // WHY: All devices on bus must use same speed
    //      400kHz is "fast mode" - good balance of speed and reliability
