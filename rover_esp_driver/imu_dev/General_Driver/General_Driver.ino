...
// advance functions for ugv ctrl. Mission Control (Reading files)
...
void loop() {
  ...
  // If we finished faster than the target period, wait
  if (loop_duration < MAIN_LOOP_PERIOD_MS) {
    delay(MAIN_LOOP_PERIOD_MS - loop_duration);
  }
  
  // Debug: Print actual loop rate every 5 seconds
  #ifdef DEBUG_LOOP_TIMING
  if (millis() - last_rate_print >= 5000) {
    actual_loop_rate = (float)loop_count / 5.0f;
    Serial.printf("Loop rate: %.1f Hz (target: %d Hz)\n", 
                  actual_loop_rate, MAIN_LOOP_RATE_HZ);
    loop_count = 0;
