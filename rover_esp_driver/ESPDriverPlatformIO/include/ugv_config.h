...
// --- --- --- IMU DEBUG --- --- ---
// Set to true to include raw (uncalibrated) magnetometer data in IMU response
// This helps diagnose magnetometer calibration issues
bool IMU_DEBUG_RAW = true;

// Raw magnetometer values (before offset correction)
int16_t mx_raw = 0, my_raw = 0, mz_raw = 0;

// Current magnetometer offsets (for reference/tuning)
// These are duplicated from IMU.cpp for visibility in JSON
int16_t mag_offset_x = 37, mag_offset_y = -28, mag_offset_z = -12;

bool MAG_AUTO_CAL = true;

// Minimum samples before calibration is considered valid
